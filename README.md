...
| **Cloud Managed MCP Server** | Registered, OAuth'd **read-only**, pinned to one cluster via `mcp-cluster-id: ab077df5-2917-45a6-b20d-e32aa611b879`, and available as the operator console. Deliberately **not** the application's data plane | Config generated by the Cloud Console for this Basic cluster; `claude mcp get` reports it connected. **Scope stated honestly in the tool section below** |
...
`mcp-cluster-id: ab077df5-2917-45a6-b20d-e32aa611b879`.

**Stated precisely, because the distinction matters.** What this project verified about the MCP
server is that it *works on a free Basic cluster* — a question the CockroachDB docs never answer,
since that page contains no tier or plan language at all. The Cloud Console's **Connect → Model
Context Protocol** tab generated the config for this Basic cluster directly, which settles it, and
the server is connected and read-only-scoped today.

**What this project did NOT do is route its build-time verification through it.** Every schema
check, row count, `EXPLAIN` and index-state query recorded in `docs/MEASUREMENTS.md` was run through
`cockroach-sql` over the Postgres wire, because that is what a scripted, reproducible measurement
needs. The MCP server is the operator console for a human asking questions in natural language; it
is genuinely configured and usable for that, and it was not the instrument behind the numbers in
this repo. Claiming otherwise would be the easiest kind of thing to get away with and the worst kind
of thing to be caught at.
